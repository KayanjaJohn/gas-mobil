...
export default function OrderScreen() {
  ...
  const [orderType, setOrderType] = useState<'swap' | 'buy'>('swap');
  const [orderSize, setOrderSize] = useState<string>('12kg');
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sizes}>
          {sizes.map((s: string) => (
            ...
            >
              {s === '6kg' && <View style={styles.popBadge}><Text style={styles.popText}>POPULAR</Text></View>}
              <View>
                <Text style={styles.sizeTitle}>{s}</Text>
                <Text style={styles.sizeSub}>{orderType === 'swap' ? 'Refill' : 'Full Kit'}</Text>
              </View>
              <Text style={styles.sizePrice}>UGX {PRICES[orderType][s].toLocaleString()}</Text>
            </TouchableOpacity>
          ))}
        </View>
